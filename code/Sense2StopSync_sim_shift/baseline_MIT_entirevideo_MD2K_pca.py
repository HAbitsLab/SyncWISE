...
def baseline_MIT_video_MD2K(window_size_sec, stride_sec, num_offsets, max_offset, window_criterion, offset_sec = 0, plot=0):
    ...
    for sub in subjects:        
        ...
        for f in flow_files:
            ...
            if plot:
                out_path = 'figures/figures_MIT_pca/corr_flow_averaged_acc_{}.png'.format(vid_name)
    ...
    result_df.to_csv('result/baseline_pca/baseline_MIT_entirevideo_MD2K_offset.csv', index=None)
    # result_df.to_csv('result/baseline_pca/baseline_MIT_entirevideo_MD2K_offset' + title_suffix + '.csv', index=None)
    print(ave_error, PV300, PV700)
    return result_df


def summarize_batch_result(offset_sec):
    file_path = './file_list_random.txt'
    summ_path = './result/batch_result_baseline_MIT_random_pca.csv'
    stride_sec = 1
    num_offsets = 20
    ...
    fout.write('window_size_sec, stride_sec, num_offsets, window_criterion, max_offset, num_videos, ave_offset, num_1000ms, num_700ms, num_300ms, PV700, PV300\n')
    for line in params:
        ...
        PV700 = num_700ms/num_videos
        PV300 = num_300ms/num_videos
        assert len(error_abs) == len(result_df)
        fout.write(','.join(map(str, [window_size_sec, stride_sec, num_offsets, window_criterion, max_offset, num_videos, ave_offset, num_1000ms, num_700ms, num_300ms, PV700, PV300]))+'\n')
    fout.close()
...
if __name__ == "__main__":
    ...
    summarize_batch_result(offset_sec)
